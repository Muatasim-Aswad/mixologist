...
export function createHeaderView() {
  ...
  header.innerHTML = String.raw`
    <div class="pb-4 sm:pb-0 flex items-center gap-4">
      ...
      <!-- Favorites Icon -->
      <div id="favorites-icon" class="relative cursor-pointer">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32" height="32" 
          class="transition-colors duration-300 ${favorites.getLocalStorage().length ? "text-red-500" : "text-gray-500"}">
          <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" fill="currentColor"/>
        </svg>
        <!-- Badge for favorite count -->
        <span id="favorites-count" 
          class="absolute -top-1 -right-2 bg-gray-500 text-white text-xs font-semibold rounded-full w-5 h-5 flex items-center justify-center 
    ...
    <div class="search-bar flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-3 w-full sm:w-auto">
      ...
      <button id="search-button" class="bg-blue-600 text-white py-2 px-6 rounded-lg hover:bg-blue-700 focus:outline-none w-full sm:w-auto">
        Random
      </button>
    </div>
  `;
